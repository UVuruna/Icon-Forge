"""Fast guard runner for Claude Code hooks (root `rules/CODE.md`,
Enforcement).

Runs the project's guard tests via `pytest.main`, prints a failure summary
to stderr, and exits **2** on failure — exit 2 is what makes a
PostToolUse/Stop hook BLOCKING. Stays fast (well under ~2 s) and
deterministic: guards only. Icon Forge has no separate behavioral test
suite (small utility, no prior automated tests) — these four guards ARE
`tests/`.
...
mode, a call for a file that is NOT `.py`/`.ps1` exits 0 immediately, before
...
# Matches test_structure_law.py's SUFFIXES — the only fast guards that could
# possibly fire on a non-.py file are config-sections (.py only) and
# structure-law (.py + .ps1, since write_shortcuts.ps1 is a real source file).
...
def _fast_exit_for_non_source() -> bool:
    """True when this is a PostToolUse call for a file that is not
    .py/.ps1 — the FAST guards (structure law, config sections) have
    nothing to check."""

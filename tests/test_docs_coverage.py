"""DOCS.md's coverage guard (root `rules/DOCS.md`, "Tiers — Which File Gets
Which Docs").
...
Root-level loose scripts (`config.py`, `run.py`) have no package -- their
`__about/` sits directly under the project root and `README.md` plays the
`___folder.md` role (root DOCS.md, "flat, root-level-only projects"
provision).

`core/write_shortcuts.ps1` is deliberately absent from TIERS -- it is not a
`.py` file, and DOCS.md's tier system/templates target this project's
dominant stack (root CLAUDE.md — Icon Forge, "Enforcement — Project
Deltas"); it stays Trivial-by-convention, described inside
`core/___core.md` and `core/__about/shortcuts.md` (its only caller).
...
TIERS: dict[str, str] = {
    # -- root: flat scripts, no package -- README.md plays the ___folder.md role --
    "config.py": STANDARD,
    "run.py": STANDARD,

    # -- core/ -- the engine ----------------------------------------------------
    "core/__init__.py": TRIVIAL,
    "core/converter.py": ALGORITHMIC,
    "core/manifest.py": STANDARD,
    "core/paths.py": STANDARD,
    "core/shortcuts.py": ALGORITHMIC,

    # -- gui/ -- the control panel ------------------------------------------------
    "gui/__init__.py": TRIVIAL,
    "gui/app.py": ALGORITHMIC,
    "gui/theme.py": STANDARD,
    "gui/worker.py": STANDARD,
}
...
# `.ps1` files are outside this guard's scope (see module docstring) --
# core/write_shortcuts.ps1 is checked by test_structure_law.py instead.
SUFFIXES = (".py",)
EXCLUDED_DIR_NAMES = {
...
def test_every_source_file_is_accounted_for():
    """No file falls through the cracks: every .py file outside tests/ is
...
def test_no_orphaned_about_or_flow_doc():
    ...
    (renamed, deleted, tier downgraded) structurally CANNOT be true once its
    subject is gone -- exactly the drift the Living Docs Rule exists to
    prevent."""
    orphans = []
...
def test_the_guard_is_actually_looking_at_this_project():
    assert len(TIERS) > 5, len(TIERS)
    assert {Path(rel).suffix for rel in TIERS} == {".py"}

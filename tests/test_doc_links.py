...
- Monorepo-root doc references (e.g. `rules/CODE.md`, `DESIGN.md`) -- DOCS.md
  requires those to be written as plain backtick text, never as markdown
  links, precisely because a project-level guard cannot assert targets
  outside the project. Any relative link that resolves outside PROJECT_ROOT
  is therefore skipped rather than asserted.
...
def test_the_guard_is_actually_looking_at_this_project():
    ...
    assert len(all_docs) > 5, len(all_docs)
